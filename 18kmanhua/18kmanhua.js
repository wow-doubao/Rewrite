    ...
    return d().wrap(function (G) {
      for (;;) {
        switch (G.prev = G.next) {
          ...
          case 8:
            /memberDetails/.test($request.url) && (g = $.toObj(g), w = g.data.suffix, x = v.enc.Utf8.parse("9a62e6977b" + w), y = v.enc.Utf8.parse("9a2d65b4a693ad0e40e5b156df25e406"), z = j(g.data.data, y, x, v), $.debug("解密后数据: " + z), A = z.replace(/"username":"[^"]+"/g, "\"username\":\"WOW\"").replace(/"password":"[^"]+"/g, "\"password\":\"TG@ios151\"").replace(/"is_vip":0/g, "\"is_vip\":1").replace(/"vip_time":null/g, "\"vip_time\":\"2099-09-09\"").replace(/"book_coin":\d+/g, "\"book_coin\":9999").replace(/"identity_id":\d+/g, "\"identity_id\":9999").replace(/"vip_hour":\d+/g, "\"vip_hour\":9999"), $.debug("修改后数据: " + A), g.data.data = i(A, y, x, v), g = JSON.stringify(g));
